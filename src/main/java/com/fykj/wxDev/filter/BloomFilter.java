...
public class BloomFilter<E> implements Serializable {
  ...
  private static final long serialVersionUID = -5856752912683829380L;
  ...
  /**
   * 通过指定误报率来构造一个过滤器。 每个元素所占的位数和哈希函数的数量会根据误报率来得出。
   *
   ...
   * @param expectedNumberOfFilterElements 指定了过滤器所能添加的元素的最大数量 to contain.
  ...
  /**
   * 根据最大元素数量和过滤器的大小来计算误报率。 方法的返回值为误报率。如果插入的元素个数小于最大值，则误报率会比返回值要小。
   *
   * @return 期望的误报率.
   */
  ...
  /**
   * 向过滤器中添加元素。 添加的元素的toString()方法将会被调用，返回的字符串作为哈希函数的输出。
   *
